...
void CGame::InitSpriteList(void)
{
	...
	if (m_LevelNumber<=MAX_LEVELS)	// Don't change music on bonus screens
	{
		if (m_LevelNumber<10) {
			tune = SMOD_TUNE1; 
		} else if (m_LevelNumber < 20) {
			tune = SMOD_TUNE2;
		} else if (m_LevelNumber < 30) {
			tune = SMOD_TUNE3;
		} else if (m_LevelNumber < 40) {
			tune = SMOD_TUNE4;
		} else if (m_LevelNumber < 50) {
			tune = SMOD_TUNE5;
		} else if (m_LevelNumber < 60) {
			tune = SMOD_TUNE6;
		} else if (m_LevelNumber < 70) {
			tune = SMOD_TUNE7;
		} else if (m_LevelNumber < 80) {
			tune = SMOD_TUNE8;
		} else if (m_LevelNumber < 90) {
			tune = SMOD_TUNE9;
		} else {
			tune = SMOD_TUNE10;
		}
